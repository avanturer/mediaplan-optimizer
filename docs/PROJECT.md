...
не тратим, чтобы недорасход и перевыполнение уравновесились.
...
стратегия заканчивает с отклонением 6,1 % по KPI, заморозка с 7,6 %.
...
миров, где отклонение по KPI не больше 20 % (порог кейса):
...
| CTR −40 % | 2,0 % / 19,0 % / 52% | 15,4 % / 10,7 % / 90% | 87% |
...
укладывается в порог кейса. Наша стратегия выполняет план по KPI в среднем
точно (2 365 при плане 2 271), укладывается в порог примерно в
девяти мирах из десяти, выигрывает у заморозки в 87% миров и в щедрых
мирах возвращает в среднем 17 % бюджета (в трети миров ноль, в самых
щедрых до 60 %). Цена этого недорасход, и мы его показываем, а не прячем: в
самых щедрых мирах KPI при полном расходе был бы выше плана вдвое, и ни одна
стратегия не удержит оба отклонения в 20 %; по обоим сразу в пороге 50 %
миров у нас против 44 % у заморозки. Режим «выжимать максимум» на тех же
мирах тратит бюджет по плану и даёт KPI на 22 % выше плана, но по метрике
кейса проигрывает заморозке, потому что перевыполнение тоже отклонение; выбор
режима это продуктовое решение (`docs/business_position.md`). Гистограммы по мирам в `docs/figures/stand_hist_*.png`:
наша стратегия сжата вокруг 12 %, заморозка расползается до 120 %.
