...
работает вместе: три демонстрации из кейса проходят, 44 теста зелёные,
...
цель на час считается по формуле Turn. Отдельно стоит детектор слома: отдача
за последние сутки против трёх предыдущих с поправкой на плановую усталость,
порог 30 %. И есть резерв: если по KPI мы опережаем план, часть остатка не
тратим, чтобы недорасход и перевыполнение уравновесились.
...
диагноз недостижимости с тремя ходами, фиксация канала, лимит CPA, коридор,
...
сравнение стратегий на 20 мирах, стенд «где ломается решение». Плюс Docker,
тесты, линтеры.

## Первые результаты

Демо 1: 1,2 млн на 21 день дают 2 271 конверсию, CPA 528 ₽, восемь каналов,
цена следующей конверсии выровнена около 1 070 ₽ во всех каналах, которые не
упёрлись в потолок. Считается за 0,1 с.

Демо 2: 50 000 кликов за 14 дней. На всех каналах достижимо за 458 тысяч. На
узком пресете из четырёх каналов отказ: максимум 37 317 кликов, связывает
...
стратегия заканчивает с отклонением 6,4 % по KPI, заморозка с 7,6 %.
Детектор сработал через 30 часов. Один мир мало что доказывает, поэтому
дальше стенд.

Стенд на 20 парных мирах, отклонение в конце кампании (заморозка / наша):

- без шока: расход 2,3 % / 11,2 %, KPI 20,1 % / 12,0 %
- CTR −40 %: расход 2,3 % / 9,3 %, KPI 18,7 % / 12,3 %
- CPM ×2: расход 1,9 % / 9,8 %, KPI 18,9 % / 12,2 %
- пауза канала: расход 4,4 % / 11,2 %, KPI 20,3 % / 12,0 %

Читать это надо так. Заморозка тратит деньги ровно по плану, но KPI гуляет на
20 %, потому что каждый мир отличается от каталога. Наша стратегия выполняет
план по KPI в среднем точно, выигрывает у заморозки в 75–85 % миров и в
щедрых мирах возвращает около 9 % бюджета. Цена этого недорасход, и мы его
показываем, а не прячем. Детектор ловит шок в 11 мирах из 12 с задержкой
27–50 часов, ложных тревог 0,08 на кампанию.
...
цены. Охват по случайному размещению, а не NBD. План по дням раскладывается
равномерно. Конверсии мгновенные. VTR по допущению. MAPE по KPI как среднее
по часам около 21 % у всех стратегий из-за разброса миров, хотя в конце
кампании 12 %. И главное: каналы абстрактные, наши CPM и CTR не являются
статистикой какой-либо площадки.

## Что пока не решили

Откат уже применённого хода не сделан; одобрение реализовано перепрогоном на
тех же случайных числах, для прототипа это честно, в продукте нужен журнал.
...
Откат хода и журнал решений в итоге кампании. Стенд деградации при смещении
каталога, когда мир сильно не похож на бенчмарки. Интеграция мира MLE #2 и
повтор стенда на нём. Проверка на людях: три медиапланера и два
трафик-менеджера. Отчёт, презентация, репетиция трёх демо.
