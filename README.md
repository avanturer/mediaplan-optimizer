...
Кампания по часам в виде «Просто». Плеер идёт по дням и сам останавливается там, где нужен человек: на переносе дороже лимита и на шоке рынка. Решение — одна кнопка, откат своего решения — тоже одна.

![Плеер: остановка на карточке, решение, шок дня 11](docs/figures/cabinet/player.gif)

Итог отвечает заказчику одним абзацем: что обещали, что получили, что дало ведение.

![Итог: обещали, получили, что дало ведение](docs/figures/cabinet/summary-short.png)

Остальные экраны — бриф, медиаплан, экран отказа с тремя вариантами, вид «Подробно» со сценариями рынка, таблицей часа и стендами — лежат в `docs/figures/cabinet/`, назначение каждого снимка — в `docs/materials.md`.
